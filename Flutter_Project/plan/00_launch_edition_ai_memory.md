...
- [x] Step 02: Define core contracts and domain baselines. Target file: `plan/step_02_define_core_contracts_and_domain_baselines.md`. Detailed logic: create the typed failure and result contract, promote all mandatory business constants, add shared enums and immutable baseline models, and lock the defaults with unit tests.
  Special mapping note: Java `Settings.defaultValue()` and the all-arguments `OverlayState` constructor were mapped to hand-written immutable Dart models with `copyWith` plus legacy-style `with...` methods; the nullable ignored update version uses a sentinel so Dart can distinguish "preserve current value" from "clear to null". Java `OneShotEffect.Type` and `AnimType` were promoted into shared Dart enums, while duration and dp defaults now live in the single `business_constants.dart` source of truth.
- [x] Step 03: Build overlay constraints and version logic. Target file: `plan/step_03_build_overlay_constraints_and_version_logic.md`. Detailed logic: implement pure-domain geometry policies, safe-area and size clamping, snap behavior, minimized-dot boundaries, semantic version comparison, and domain tests for malformed inputs and edge cases.
  Special mapping note: Java `OverlayConstraints` static helpers were translated into pure Dart static methods that keep the same integer clamp, safe-inset max-bound, 80% truncation, and nearest-edge tie behavior; minimized-dot sizing and sized-position helpers were promoted from the legacy ViewModel into the domain policy for later controller reuse. Java `VersionNameComparator` null trimming, prefix stripping, qualifier removal, regex digit extraction, and `NumberFormatException` fallback were mirrored with nullable Dart inputs and explicit `FormatException` handling.
- [x] Step 04: Implement settings persistence and config transfer. Target file: `plan/step_04_implement_settings_persistence_and_config_transfer.md`. Detailed logic: add Hive-backed persistence, repository contracts, import/export payloads, clipboard and share ports, atomic import failure behavior, export priority handling, and tests for merge rules.
  Special mapping note: Java `SharedPreferencesSettingsRepository` was mapped to a Hive JSON-record repository that still loads missing settings as `Settings.defaultValue()`, reconstructs the last overlay from saved width/height/x/y plus current settings, and trims empty ignored update versions to `null`. Java `MainActivity.exportConfig()`/`importConfig()` and `OverlayViewModel.applyImportedState()` were translated into a versioned config-transfer payload/service: export prefers runtime state before persisted state, invalid JSON/validation failures write nothing, import preserves the current runtime visibility and interaction flags while forcing `transparentMode` to `false`.
- [x] Step 05: Create the update checking data flow. Target file: `plan/step_05_create_update_checking_data_flow.md`. Detailed logic: build the remote release client, DTO mapping, current-version comparison, ignored-version handling, manual versus automatic semantics, and tests that preserve the legacy update behavior.
  Special mapping note: Java `GithubReleaseClient.fetchLatestRelease()` was mapped to a configurable Dio-backed GitHub release data source with the same endpoint shape, headers, timeout intent, JSON fields, APK-asset scan order, and explicit network/parse failures. Java `MainActivity.handleLatestRelease()` update decisions were mapped into `UpdateRepositoryImpl`: manual and automatic up-to-date results are distinct, ignored versions suppress only until a higher normalized release appears, and missing Android APK assets degrade to a release-page action plan instead of performing installation.
- [x] Step 06: Build application controllers and command flow. Target file: `plan/step_06_build_application_controllers_and_command_flow.md`. Detailed logic: create Riverpod controllers for settings, overlay, and updates, emit typed one-shot commands, preserve hide and transparency timing rules, and cover mandatory behavior with controller tests.
  Special mapping note: Java `LiveData<OneShotEffect>` and consumed one-shot events were mapped to a typed broadcast `ApplicationCommandBus`, while durable `LiveData` state became Riverpod `AsyncNotifier` state objects. Java `Handler.postDelayed()` hide and transparency-restore branches were translated to an injected `ControllerScheduler`, preserving the 320ms hide completion delay and cancellable auto-restore behavior without test sleeps.
